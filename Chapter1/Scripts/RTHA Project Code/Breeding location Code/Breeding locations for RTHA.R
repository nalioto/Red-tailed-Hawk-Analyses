## Breeding locations of 9 RTHA from 2021
## Nick Alioto
## 01/06/2022

## Set working directory
setwd("C:/Users/Nick/OneDrive/Desktop/R Code + Script/RTHA CSV files/2021 RTHA CSV")
...
a <- read.csv("morpheus.csv", header=TRUE) 
b <- read.csv("rowan.csv", header=TRUE) 
c <- read.csv("Rip.csv", header=TRUE)
d <- read.csv("sam.csv", header=TRUE)
e <- read.csv("trinity.csv", header = TRUE)
f <- read.csv("patagium.csv", header = TRUE)
g <- read.csv("kirby.csv", header = TRUE)
h <- read.csv("jack.csv", header = TRUE)
i <- read.csv("herald.csv", header = TRUE)
rm()
## Subset each time frame we want to plot tz = timezone
...
f$timestamp <- ymd_hms(f$timestamp,tz='GMT')
g$timestamp <- ymd_hms(g$timestamp,tz='GMT')
h$timestamp <- ymd_hms(h$timestamp,tz='GMT')
i$timestamp <- ymd_hms(i$timestamp,tz='GMT')

str()
rm(g)
##Combine all CSV's into one single file
raw <- rbind(a,b,c,d,e,f,h,i)

## Subset the times we want to plot
raw <- subset(raw, timestamp>="2021-05-01 00:00:01" & timestamp<="2021-06-01 00:00:01")
...
png("Breeding_locations_RTHA_01/2022.png", width=2000, height=1500, res=300) # saving map as a .png file with name, dimensions, and resolution
map.plot = ggplot(rtha, aes(long, lat)) + theme_bw() +
  ...
  geom_path(aes(x=long,y=lat,group=tagid,color=tagid),size= 2) + # adding movement paths of birds, grouping and coloring by bird ID and specify width of the line
  ...
  coord_fixed(xlim=c(-95,-70), ylim=c(42.5,58), ratio=1.5) # specifying the limits of the plotting area in lat/long and the mapping ratio (will distort projection)
