...
# 2021 birds
{
  ...
  H30 <- read.csv(Hawks.3[7]) # Added in Roger that down here as he was in the 2023 cohort
...
# covert to a move object - make sure to project the coordinates UTM, Northing and Easting -- * No Jack for Spring *
{
  ...
  rogerthat   <- move(x=H30$location.long, y=H30$location.lat,
                      time= as.POSIXct(H30$timestamps, format= "Y%-%m-d% %H:%M:%S", tz= "UTC"),
                      proj= CRS("+proj=longlat +datum=WGS84 +datum=WGS84"),
                      data = H30, animal = H30$individual.local.identifier, sensor= H30$sensor.type)
  
  
  baker   <- move(x=H23$location.long, y=H23$location.lat,
  ...
  campa   <- move(x=H24$location.long, y=H24$location.lat,
                  time= as.POSIXct(H24$timestamps, format= "Y%-%m-d% %H:%M:%S", tz= "UTC"),
                  proj= CRS("+proj=longlat +datum=WGS84 +datum=WGS84"),
                  data = H24, animal = H24$individual.local.identifier, sensor= H24$sensor.type) 
  
  disco   <- move(x=H25$location.long, y=H25$location.lat,
                  time= as.POSIXct(H25$timestamps, format= "Y%-%m-d% %H:%M:%S", tz= "UTC"),
                  proj= CRS("+proj=longlat +datum=WGS84 +datum=WGS84"),
                  data = H25, animal = H25$individual.local.identifier, sensor= H25$sensor.type)
  
  mercury   <- move(x=H26$location.long, y=H26$location.lat,
                    time= as.POSIXct(H26$timestamps, format= "Y%-%m-d% %H:%M:%S", tz= "UTC"),
                    proj= CRS("+proj=longlat +datum=WGS84 +datum=WGS84"),
                    data = H26, animal = H26$individual.local.identifier, sensor= H26$sensor.type)
  
  owen    <- move(x=H27$location.long, y=H27$location.lat,
                  time= as.POSIXct(H27$timestamps, format= "Y%-%m-d% %H:%M:%S", tz= "UTC"),
                  proj= CRS("+proj=longlat +datum=WGS84 +datum=WGS84"),
                  data = H27, animal = H27$individual.local.identifier, sensor= H27$sensor.type) 
  
  paprika   <- move(x=H28$location.long, y=H28$location.lat,
                    time= as.POSIXct(H28$timestamps, format= "Y%-%m-d% %H:%M:%S", tz= "UTC"),
                    proj= CRS("+proj=longlat +datum=WGS84 +datum=WGS84"),
                    data = H28, animal = H28$individual.local.identifier, sensor= H28$sensor.type) 
  
  reno   <- move(x=H29$location.long, y=H29$location.lat,
                 time= as.POSIXct(H29$timestamps, format= "Y%-%m-d% %H:%M:%S", tz= "UTC"),
                 proj= CRS("+proj=longlat +datum=WGS84 +datum=WGS84"),
                 data = H29, animal = H29$individual.local.identifier, sensor= H29$sensor.type) 
  
}

# Remove df's for memory in R, only need the move objects moving forward
rm(H1,H2,H3,H4,H5,H6,H7,H8,H9,H10,H11,H12,H13,H14,H15,H16,H17,H18,H19,H20,H21,H22,H23,H24,H25,H26,
   H27,H28,H29,H30, Hawks.21,Hawks.22, Hawks.23, Hawks.24)
...
### selecting a range of days we want to look at i.e(Fall or Spring Migration Periods)
{
  ...
  rogerthat.fall <- rogerthat[as.Date(timestamps(rogerthat)) >= as.Date("2023-10-20")& as.Date(timestamps(rogerthat)) <= as.Date("2023-11-04")]
  
  # 2024 
  patagium.fall.24 <- patagium[as.Date(timestamps(patagium)) >= as.Date("2024-10-10")& as.Date(timestamps(patagium)) <= as.Date("2024-11-08")]
  trinity.fall.24 <- trinity[as.Date(timestamps(trinity)) >= as.Date("2024-10-03")& as.Date(timestamps(trinity)) <= as.Date("2024-10-18")]
  mackinaw.fall.24 <- mackinaw[as.Date(timestamps(mackinaw)) >= as.Date("2024-10-09")& as.Date(timestamps(mackinaw)) <= as.Date("2024-11-30")]
  cypress.fall.24 <- cypress[as.Date(timestamps(cypress)) >= as.Date("2024-10-04")& as.Date(timestamps(cypress)) <= as.Date("2024-10-26")]
  lightfoot.fall.24 <- lightfoot[as.Date(timestamps(lightfoot)) >= as.Date("2024-10-27")& as.Date(timestamps(lightfoot)) <= as.Date("2024-11-21")]
  rogerthat.fall.24 <- rogerthat[as.Date(timestamps(rogerthat)) >= as.Date("2024-10-10")& as.Date(timestamps(rogerthat)) <= as.Date("2024-10-29")]
  baker.fall <- baker[as.Date(timestamps(baker)) >= as.Date("2024-10-27")& as.Date(timestamps(baker)) <= as.Date("2024-12-04")]
  campa.fall <- campa[as.Date(timestamps(campa)) >= as.Date("2024-09-15")& as.Date(timestamps(campa)) <= as.Date("2024-09-26")]
  disco.fall <- disco[as.Date(timestamps(disco)) >= as.Date("2024-09-05")& as.Date(timestamps(disco)) <= as.Date("2024-10-15")]
  mercury.fall <- mercury[as.Date(timestamps(mercury)) >= as.Date("2024-09-23")& as.Date(timestamps(mercury)) <= as.Date("2024-10-16")]
  owen.fall <- owen[as.Date(timestamps(owen)) >= as.Date("2024-10-08")& as.Date(timestamps(owen)) <= as.Date("2024-10-18")]
  paprika.fall <- paprika[as.Date(timestamps(paprika)) >= as.Date("2024-08-27")& as.Date(timestamps(paprika)) <= as.Date("2024-10-24")]
  reno.fall <- reno[as.Date(timestamps(reno)) >= as.Date("2024-06-09")& as.Date(timestamps(reno)) <= as.Date("2024-06-14")]
...
{
  rowan.fall <- spTransform(rowan.fall, CRSobj = "+proj=utm +zone=16 +datum=WGS84 +units=m +no_defs +type=crs")
  ...
  rogerthat.fall <- spTransform(rogerthat.fall, CRSobj = "+proj=utm +zone=16 +datum=WGS84 +units=m +no_defs +type=crs")
  
  patagium.fall.24 <- spTransform(patagium.fall.24, CRSobj = "+proj=utm +zone=16 +datum=WGS84 +units=m +no_defs +type=crs")
  
  trinity.fall.24 <- spTransform(trinity.fall.24, CRSobj = "+proj=utm +zone=16 +datum=WGS84 +units=m +no_defs +type=crs")
  
  mackinaw.fall.24 <- spTransform(mackinaw.fall.24, CRSobj = "+proj=utm +zone=16 +datum=WGS84 +units=m +no_defs +type=crs")
  
  cypress.fall.24 <- spTransform(cypress.fall.24, CRSobj = "+proj=utm +zone=16 +datum=WGS84 +units=m +no_defs +type=crs")
  
  lightfoot.fall.24 <- spTransform(lightfoot.fall.24, CRSobj = "+proj=utm +zone=16 +datum=WGS84 +units=m +no_defs +type=crs")
  
  rogerthat.fall.24 <- spTransform(rogerthat.fall.24, CRSobj = "+proj=utm +zone=16 +datum=WGS84 +units=m +no_defs +type=crs")
  
  baker.fall <- spTransform(baker.fall, CRSobj = "+proj=utm +zone=16 +datum=WGS84 +units=m +no_defs +type=crs")
  
  campa.fall <- spTransform(campa.fall, CRSobj = "+proj=utm +zone=16 +datum=WGS84 +units=m +no_defs +type=crs")
  
  disco.fall <- spTransform(disco.fall, CRSobj = "+proj=utm +zone=16 +datum=WGS84 +units=m +no_defs +type=crs")
  
  mercury.fall <- spTransform(mercury.fall, CRSobj = "+proj=utm +zone=16 +datum=WGS84 +units=m +no_defs +type=crs")
  
  owen.fall <- spTransform(owen.fall, CRSobj = "+proj=utm +zone=16 +datum=WGS84 +units=m +no_defs +type=crs")
  
  paprika.fall <- spTransform(paprika.fall, CRSobj = "+proj=utm +zone=16 +datum=WGS84 +units=m +no_defs +type=crs")
  
  reno.fall <- spTransform(reno.fall, CRSobj = "+proj=utm +zone=16 +datum=WGS84 +units=m +no_defs +type=crs")
...
# Check the time lag in between points for each bird if need be will reduce dbbmm run time
...
# Create a time step object to put into the model for each individual bird
{
  ...
  ts35 <- median(timeLag(rogerthat.fall, "mins"))
  ts36 <- median(timeLag(patagium.fall.24, "mins"))
  ts37 <- median(timeLag(trinity.fall.24, "mins"))
  ts38 <- median(timeLag(mackinaw.fall.24, "mins"))
  ts39 <- median(timeLag(cypress.fall.24, "mins"))
  ts40 <- median(timeLag(lightfoot.fall.24, "mins"))
  ts41 <- median(timeLag(rogerthat.fall.24, "mins"))
  ts42 <- median(timeLag(baker.fall, "mins"))
  ts43 <- median(timeLag(campa.fall, "mins"))
  ts44 <- median(timeLag(disco.fall, "mins"))
  ts45 <- median(timeLag(mercury.fall, "mins"))
  ts46 <- median(timeLag(owen.fall, "mins"))
  ts47 <- median(timeLag(paprika.fall, "mins"))
  ts48 <- median(timeLag(reno.fall, "mins"))
  
  
}

# Create a Raster to project all of our birds onto so that they all fit
ext <- 830000 # If a bird doesn't fit, increase the extent the raster even more

# Use Criket this bird breeds in Manitoba and has a large spatial extent
Ra1 <- raster(extent(cricket.fall)+c(-ext,ext,-ext,ext), resolution=1000, crs = crs(cricket.fall), vals=NULL)
str(Ra1)
# ?raster
# originally ran at resolution 2500 possibly to low? 
# Now at 1000 = 1km - not much of a difference

# STEP 8 Run dBBMM's ----
# Create a dBBMM object for each bird and then plot the model- should be 48

{
  ...
  dBB.rogerthat <- brownian.bridge.dyn(rogerthat.fall, window.size = 25, #translates to a sliding window ~ 1 day
                                       margin = 9,
                                       location.error = 5,
                                       raster = Ra1,
                                       time.step= 15)
  
  dBB.patagium4 <- brownian.bridge.dyn(patagium.fall.24, window.size = 25, #translates to a sliding window ~ 1 day
                                       margin = 9,
                                       location.error = 5,
                                       raster = Ra1,
                                       time.step= ts36/15)
  
  dBB.trinity4 <- brownian.bridge.dyn(trinity.fall.24, window.size = 25, #translates to a sliding window ~ 1 day
                                      margin = 9,
                                      location.error = 5,
                                      raster = Ra1,
                                      time.step= ts37/15)
  
  dBB.mackinaw3 <- brownian.bridge.dyn(mackinaw.fall.24, window.size = 25, #translates to a sliding window ~ 1 day
                                       margin = 9,
                                       location.error = 5,
                                       raster = Ra1,
                                       time.step= ts38/15)
  
  dBB.cypress2 <- brownian.bridge.dyn(cypress.fall.24, window.size = 25, #translates to a sliding window ~ 1 day
                                      margin = 9,
                                      location.error = 5,
                                      raster = Ra1,
                                      time.step= 15)
  
  dBB.lightfoot2 <- brownian.bridge.dyn(lightfoot.fall.24, window.size = 25, #translates to a sliding window ~ 1 day
                                        margin = 9,
                                        location.error = 5,
                                        raster = Ra1,
                                        time.step= ts40/15)
  
  dBB.rogerthat2 <- brownian.bridge.dyn(rogerthat.fall.24, window.size = 25, #translates to a sliding window ~ 1 day
                                        margin = 9,
                                        location.error = 5,
                                        raster = Ra1,
                                        time.step= 15)
  
  dBB.baker <- brownian.bridge.dyn(baker.fall, window.size = 25, #translates to a sliding window ~ 1 day
                                   margin = 9,
                                   location.error = 5,
                                   raster = Ra1,
                                   time.step= ts42/15)
  
  dBB.campa <- brownian.bridge.dyn(campa.fall, window.size = 25, #translates to a sliding window ~ 1 day
                                   margin = 9,
                                   location.error = 5,
                                   raster = Ra1,
                                   time.step= ts43/15)
  
  dBB.disco <- brownian.bridge.dyn(disco.fall, window.size = 25, #translates to a sliding window ~ 1 day
                                   margin = 9,
                                   location.error = 5,
                                   raster = Ra1,
                                   time.step= ts44/15)
  
  dBB.mercury <- brownian.bridge.dyn(mercury.fall, window.size = 25, #translates to a sliding window ~ 1 day
                                     margin = 9,
                                     location.error = 5,
                                     raster = Ra1,
                                     time.step= ts45/15)
  
  dBB.owen <- brownian.bridge.dyn(owen.fall, window.size = 25, #translates to a sliding window ~ 1 day
                                  margin = 9,
                                  location.error = 5,
                                  raster = Ra1,
                                  time.step= ts46/15)
  
  dBB.paprika <- brownian.bridge.dyn(paprika.fall, window.size = 25, #translates to a sliding window ~ 1 day
                                     margin = 9,
                                     location.error = 5,
                                     raster = Ra1,
                                     time.step= ts47/15)
  
  dBB.reno <- brownian.bridge.dyn(reno.fall, window.size = 25, #translates to a sliding window ~ 1 day
                                  margin = 9,
                                  location.error = 5,
                                  raster = Ra1,
                                  time.step= 15)
...
# Step 9 Utilization Distributuions ----
# Get the Utilization Distribution for each bird (48 TOTAL MIGRATION EVENTS)
...
# 2021 fall migrations
{
  ...
  # 2024 fall migrations
  UDpatagium4 <- getVolumeUD(dBB.patagium4)
  UDtrinity4 <- getVolumeUD(dBB.trinity4)
  UDmackinaw3 <- getVolumeUD(dBB.mackinaw3)
  UDcypress2 <- getVolumeUD(dBB.cypress2)
  UDlightfoot2 <- getVolumeUD(dBB.lightfoot2)
  UDrogerthat2 <- getVolumeUD(dBB.rogerthat2)
  UDbaker <- getVolumeUD(dBB.baker)
  UDcampa <- getVolumeUD(dBB.campa)
  UDdisco <- getVolumeUD(dBB.disco)
  UDmercury <- getVolumeUD(dBB.mercury)
  UDowen <- getVolumeUD(dBB.owen)
  UDpaprika <- getVolumeUD(dBB.paprika)
  UDreno <- getVolumeUD(dBB.reno)
  
}  

##################################
##### Duration (Weighted Raster) #
#####     Calculations           #
##################################

# Step 10 Multiply each birds UD by the total migration duration in days
# (calculated in previous script script)

# 2021 Durations

{
  rowan.f.dur <-    (UDrowan * 41)        
  ...
  mh1.f.dur <- (UDmh1 * 28)
  rogerthat.f.dur <- (UDrogerthat *15)
  
  # 2024 Durations
  patagium.f.dur4 <- (UDpatagium4 * 29)
  trinity.f.dur4 <- (UDtrinity4 * 15 )
  mackinaw.f.dur3 <- (UDmackinaw3 * 53)
  cypress.f.dur2 <- (UDcypress2 * 22)
  lightfoot.f.dur2 <- (UDlightfoot2 * 25)
  rogerthat.f.dur2 <- (UDrogerthat2 *19)
  baker.f.dur <- (UDbaker * 38)
  campa.f.dur <- (UDcampa * 11)
  disco.f.dur <- (UDdisco * 40)
  mercury.f.dur <- (UDmercury * 30)
  owen.f.dur <- (UDowen * 10)
  paprika.f.dur <- (UDpaprika * 51)
  reno.f.dur <- (UDreno * 5)
  
}

# Step 11 Bringing it all together ----
# Calculate a cumulative Utilization Distribution for all birds across all fall migs
# Argument = raster layer with a corresponding utilization distribution (one for each RTHA)

# stack all UD's together
fall.stk.test2 <-  stack(rowan.f.dur,herald.f.dur,morpheus.f.dur,patagium.f.dur,rip.f.dur,sam.f.dur,trinity.f.dur,patagium.f.dur2,
                         ...
                         cypress.f.dur,hc2.f.dur,lightfoot.f.dur,mh1.f.dur,rogerthat.f.dur, patagium.f.dur4, trinity.f.dur4,
                         mackinaw.f.dur3,cypress.f.dur2,lightfoot.f.dur2,rogerthat.f.dur2,baker.f.dur,campa.f.dur,disco.f.dur,
                         mercury.f.dur, owen.f.dur,paprika.f.dur,reno.f.dur)


# Sum total values for all UDs
fall.mig.sum.test2 <- sum(fall.stk.test2 )  # /(nmigs *ndays)

plot(fall.mig.sum.test2)

# TEST
test3 <- fall.mig.sum.test2 # Extract the 99% contour
test3[test3 > 1569] = NA    # Get the max value from the raster
plot(test3)

# Save the UD as a Raster for all Fall migration routes 
writeRaster(test3, filename='Chapter.1/Rasters/fall.all.24.Updated.tiff', overwrite=TRUE)

# This goes into ARCGIS for further calculations in the manuscript
...
# Put your individual UDs into a list, grab all rasters that start with UD from ENV
uud_list <- mget(ls(pattern = "^UD"))
...
writeRaster(propor_rast,
            filename = "Chapter.1/Rasters/fall_proportion_raster.tif",
            overwrite = TRUE)

# Bring this raster into ArcGIS as well
